...
***There are 6 observations in the dn_lq_ak data frame. There are 11 variables: address.x, city.x, state, zip.x, longitude.x, latitude.x address.y, city.y, zip.y, longitude.y, and latitude.y.
...
***The minimum distances are 5.197 miles, 2.035, and 5.998.***
...
dn_lq_ak_mindist %>% summarize(mean_dist = mean(closest))
...
dn_lq_nc_mindist %>% summarize(mean_dist = mean(closest))
```
...
dn_lq_tx_mindist %>% summarize(mean_dist = mean(closest))
```
...
```{r}
ggplot(dn_lq_ny_mindist, aes(x = closest)) +
  geom_histogram()

dn_lq_ny_mindist %>% summarize(mean_dist = mean(closest))
```

12. Among the states you examined, where is Mitch Hedberg's joke most likely to hold true?
    Explain your reasoning.

***Alaska is the state where Hedberg's joke is most likely true. The average distance between Denny's and La Quinta's is the closest.
